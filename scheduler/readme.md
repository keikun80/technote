...
#### 배경 
사내 통합 스케줄러 서버의 유지보수계약이 종료되면서  각 애자일 그룹별로 배치 스케줄러 구축 요구가 제기 되었습니다. 

#### 문제
1. 스케줄러 장애 발생 시 모든 배치 작업이 중지 된다.
2. HA 구성이 수동으로 되어 있어 전환에 사람의 인지가 필요하고 작업 시간도 필요하다.
...
위의 그림에서 중지시간을 줄이기 위한 아이디어로 AWS Auto scale group을 이용한 auto healing을 적용하기로 한 설계를 구상했습니다.
...
##### 5. 비용비교 및 성과
기존에 2대의 ETL서버를 이용한 구성에서는 $1,716/월  + 라이선스 비용으로 구성되어 있었지만 
젠킨스를 이용한 내재화로 라이선스 비용이 감소하고 운영 안정성을 위한 그룹별 독립 서버로 구성하는 관계로 
인프라  비용은 $2,330 /월 로 기존보다 $614/월 증가 했습니다. 하지만 월 $3,000 정도의 유비관리 비용까지 계산을 하게 되면 
총액은 $2,380 정도의 금액을 절약하게 되었습니다. 전체적 유지 비용을 낮추고 RTTM, RTO를 실시간에 가깝게 달성한 것이 이 작업의 성과 입니다. 

**젠킨스 스케줄러 서버팜의 월 예상비용**
